...
export class FlashcardService {
  ...
  addFlashcard(question: string, answer: string, setId: number): Observable<any> {
    ...
    return this.http.post<any>(`${this.baseUrl}/flashcards`, { question, answer, setId }, { headers });
  }

  updateSet(id: number, name: string): Observable<any> {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}`);
    return this.http.put<any>(`${this.baseUrl}/sets/${id}`, { name }, { headers });
  }

  deleteSet(id: number): Observable<void> {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}`);
    return this.http.delete<void>(`http://localhost:3000/sets/${id}`, { headers });
  }
  ...
  addMultipleFlashcards(flashcards: { question: string; answer: string; setId: number }[]): Observable<any> {
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`,
    });

    return this.http.post<any>(`${this.baseUrl}/flashcards/bulk`, { flashcards }, { headers });
  }
}
